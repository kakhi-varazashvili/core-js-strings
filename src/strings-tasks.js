...
function isPalindrome(str) {
  const reverseStr = str
    ...
    .reverse()
    .join('');
  return str.replace(/[^a-z]/gi, '').toLocaleLowerCase() === reverseStr;
}
...
function findLongestWord(sentence) {
  ...
  let indexLongest = 0;
  for (let i = 1; i < sentenceArr.length; i += 1) {
    if (sentenceArr[indexLongest].length < sentenceArr[i].length) {
      indexLongest = i;
    }
  }
  return sentenceArr[indexLongest];
}
...
function extractNameFromTemplate(value) {
  ...
  return `${valueArr[valueArr.length - 2]} ${valueArr[
    valueArr.length - 1
  ].substring(0, valueArr[valueArr.length - 1].length - 1)}`;
}
...
function getCardId(value) {
  ...
  const suitIndex = SUIT.indexOf(sourceSuit);
  const cardIndex = CARDS.indexOf(sourceCard);
